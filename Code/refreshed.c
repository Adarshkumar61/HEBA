...
// -------------------- Servo Config --------------------
#define SERVO_MIN  110   // PCA9685 tick for ~500us
#define SERVO_MAX  510   // PCA9685 tick for ~2500us
...
const int IN3 = 23;
const int IN4 = 5;
// ENA, ENB tied to 5V for full speed (hardware)

// -------------------- Ultrasonic ----------------------
const int TRIG_PIN = 26;
const int ECHO_PIN = 25;   // echo must go via 5V->3.3V divider
...
RobotMode currentMode = MODE_CLEANING;  // default: cleaning

// -------------------- RTC Schedule --------------------
// Change these for your desired medicine time
int medHour   = 9;
int medMinute = 0;
int lastMedDayRun = -1;   // to avoid running multiple times in same day

// (Optional) cleaning schedule (we keep always ON for now)
...
// ------------------------------------------------------
// Utility: Ultrasonic distance in cm
// ------------------------------------------------------
long getDistanceCM() {
...
// ------------------------------------------------------
// Arm poses (simple demo values - tune for your arm)
// ------------------------------------------------------
void armHome() {
...
void armPickPose() {    // position near medicine/water
...
void armServePose() {   // extend hand towards patient
...
// Simple wiper oscillation when cleaning
void updateWiper() {
...
// ------------------------------------------------------
// Medicine routine (automatic or manual)
// ------------------------------------------------------
void runMedicineRoutine() {
  ...
  // Move little forward from base (optional)
  motorsForward();
  delay(800);
  motorsStop();
  ...
  armPickPose();
  delay(1200);
  openGripper();   // assume bottle in gripper area
  delay(600);
  closeGripper();
  delay(600);
...
// ------------------------------------------------------
// Water routine (similar pattern but simpler)
// ------------------------------------------------------
void runWaterRoutine() {
  currentMode = MODE_MEDICINE;  // same behaviour base
  setLEDs(false, false, true);
...
// ------------------------------------------------------
// Obstacle-avoid cleaning behaviour
// ------------------------------------------------------
void updateCleaningBehaviour() {
  long d = getDistanceCM();

  // If ultra close -> emergency stop
  if (d > 0 && d <= 5) {
    motorsStop();
    wiperUpPose();
    setLEDs(false, false, true);
...
// ------------------------------------------------------
// Check RTC for medicine time
// ------------------------------------------------------
void checkMedicineSchedule() {
  if (!rtc.begin()) return;   // safety
...
void setup() {
  ...
  // Uncomment once to set RTC to compile time, then comment again
...
// ------------------------------------------------------
void loop() {
  // Check scheduled medicine using RTC
  ...
  switch (currentMode) {
    ...
    case MODE_MEDICINE:
      // already handled inside runMedicineRoutine
      // nothing here
      break;

    case MODE_HELP:
      // just wait - no auto move
      break;
  }
