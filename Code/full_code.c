...
// Brain: ESP32 DevKit
// Features:
// - 5DOF arm (5 x SG90 servos)
// - 1 x Wiper servo for floor cleaning
// - 2WD base (2 BO motors + L298N driver)
// - DS3231 RTC for schedule (medicine + cleaning time)
// - 16x2 I2C LCD for status
// - HC-SR04 ultrasonic (front obstacle detection)
// - Buttons: Water, Medicine, Help, Cancel
// - Buzzer + LEDs: Idle, Cleaning, Alert
...
// ---------- LCD & RTC ----------
#define LCD_ADDR 0x27      // If no display, try 0x3F
...
// Extra servo: floor wiper
const int SERVO_WIPER_PIN    = 23;

// L298N motor driver pins (2 BO motors)
...
// ENA / ENB assumed jumpered to 5V on L298N for full speed

// Buttons (using internal pull-ups)
const int BTN_WATER_PIN   = 32;
const int BTN_MED_PIN     = 33;
const int BTN_HELP_PIN    = 34;  // input-only OK
const int BTN_CANCEL_PIN  = 35;  // input-only OK

// Buzzer & LEDs
const int BUZZER_PIN      = 4;
const int LED_IDLE_PIN    = 2;   // note: boot pin, unplug LED if upload issues
const int LED_CLEAN_PIN   = 15;  // boot pin
const int LED_ALERT_PIN   = 0;   // boot pin

// HC-SR04 ultrasonic
const int TRIG_PIN        = 12;  // output
const int ECHO_PIN        = 39;  // input-only pin (with external voltage divider)
...
Servo servoWrist;
Servo servoGripper;
Servo servoWiper;  // floor wiper servo
...
// ---------- Schedule (you can change times here) ----------
const int MED_HOUR    = 9;   // 09:00 = medicine time
const int MED_MINUTE  = 0;

const int CLEAN_HOUR   = 10; // 10:00 = cleaning time
...
// Some pre-defined poses (TUNE these for your physical arm)
void armHomePose() {
...
// ---------- Wiper servo helpers (TUNE angles) ----------
void wiperDown() {
  // Angle where wiper touches the floor
  // Start with ~70 and adjust after testing
  servoWiper.write(70);
  delay(400);
}

void wiperUp() {
  // Angle where wiper is lifted
  // Start with ~0 and adjust after testing
  servoWiper.write(0);
  delay(400);
}
...
void doCleaningRoutine() {
  ...
  // 2) Move in a small pattern to simulate cleaning
  // You MUST tune these times according to your robot speed.
  driveFor(motorsForward, 800);    // go forward
  driveFor(motorsTurnRight, 400);  // turn right a bit
  driveFor(motorsBackward, 800);   // go backward
  driveFor(motorsTurnLeft, 400);   // turn left back
  motorsStop();

  // 3) Raise floor wiper
  wiperUp();

  // 4) Optional: arm wipe motion over bed/table
  armCleanPose1();
  for (int i = 0; i < 2; i++) {
    armCleanPose2();
    delay(700);
...
void doWaterRoutine() {
  ...
  // Present water
  armWaterDeliverPose();
  delay(2000);  // time to "take" cup
  armHomePose();

  driveFor(motorsBackward, 800);  // return
  motorsStop();
...
void doHelpAlert() {
  ...
  // Repeating beep until cancel or 10 cycles
  for (int i = 0; i < 10; i++) {
...
void handleButtons() {
  ...
  if (cancelState == LOW && lastCancelState == HIGH) {
    // Cancel alert or any routine
    motorsStop();
    armHomePose();
    wiperUp(); // ensure wiper up
...
void setup() {
  ...
  servoWiper.attach(SERVO_WIPER_PIN,      500, 2400);

  armHomePose();
  wiperUp(); // make sure wiper is in up position
...
void loop() {
  ...
  // Show time & state on LCD (refresh every ~1s when IDLE)
